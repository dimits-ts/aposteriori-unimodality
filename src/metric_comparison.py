...
the *within-group* gain it implies, allowing for a fair comparison with apunim.
...
def simulate_group_variance(
    n_items, n_ann, minority_frac, rng, sigma_lo=0.8, sigma_hi=2.5
):
    ...
    return (
        np.clip(np.round(rng.normal(base[None, :], sd[:, None])), 1, N_LEVELS),
        groups,
    )
...
def simulate_inherent(n_items, n_ann, minority_frac, rng, sigma=SIGMA):
    ...
    return (
        np.clip(
            np.round(rng.normal(np.where(pole, 2.0, 4.0), sigma)), 1, N_LEVELS
        ),
        groups,
    )
...
def method_apunim(matrix, groups, seed):
    ...
    if not res:
        return 0.0, 1.0
    best = min(
        res.items(), key=lambda kv: kv[1].pvalue
    )  # Bonferroni over levels
...
def _delta_alpha(matrix, groups):
    ...
    within = [
        a
        for a in (_alpha(matrix[groups == g]) for g in np.unique(groups))
        if not np.isnan(a)
    ]
    return (
...
def _frac_explained(matrix, groups):
    ...
    for c in range(matrix.shape[1]):
        ...
        eligible += 1
        hits += all(
            apunim.dfu(col[groups == g], bins=N_LEVELS) <= 0 for g in levels
        )
...
def _one(job):
    ...
    return [
        (
            ...
            st,
            pv,
            int(pv < ALPHA) if pv == pv else 0,
        )
        for meth, st, pv in rows
    ]
...
def run(out_csv, n_items, n_reps, workers):
    jobs = [
        ("polarization", n, mi, d, r)
        for n, mi in CONDITIONS
        for d in DELTAS
        for r in range(n_reps)
    ]
    jobs += [
        (k, 80, 0.5, 2.5, r)
        for k in ("group-variance", "inherent", "placebo")
        for r in range(n_reps)
    ]
    ...
    with open(out_csv, "w", newline="") as fh:
        ...
        with ProcessPoolExecutor(
            max_workers=workers, initializer=_init, initargs=(cfg,)
        ) as ex:
...
METHOD_ORDER = [
    "apunim (fixed binning)",
    "apunim (as shipped, 1.0.5)",
...
def plot(rows, methods, out_path):
    ...
    fig, axes = plt.subplots(
        1, len(CONDITIONS), figsize=(9.2, 2.6), sharey=True
    )
    titles = [
        "80 annotators/item, balanced",
        "20 annotators/item, balanced",
            ...
            y = [
                np.mean(
                    [
                        r["detected"]
                        for r in rows
                        if r["scenario"] == "polarization"
                        and r["method"] == m
                        and r["n_ann"] == n_ann
                        and r["minority"] == minority
                        and r["delta"] == d
                    ]
                    or [np.nan]
                )
...
def main(
    cache_dir, graph_output_dir, label, n_items, n_reps, workers, plot_only
):
    _CFG.update(label=label, n_items=n_items)

    # The cache filename is deterministic for the calculation settings that
    # affect the simulation. If it exists, skip the expensive calculation and
    ...
    if cache_csv.exists():
        ...
        raise FileNotFoundError(
            f"--plot-only was requested, but no cached results exist at {cache_csv}"
        )
    else:
        run(cache_csv, n_items, n_reps, workers)

    rows = list(csv.DictReader(open(cache_csv)))
    for r in rows:
        r["delta"] = float(r["delta"])
        r["n_ann"] = int(r["n_ann"])
        r["minority"] = float(r["minority"])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--graph-output-dir", required=True, help="Directory for graphs."
    )
    parser.add_argument(
        "--label",
        default="apunim",
