...
\begin{document}
\maketitle
%TODO: redact package name
%TODO: Mention model training approaches in prior work
...
\begin{figure*}[t]
	\centering
	\includegraphics[width=\linewidth]{investigation.drawio.png}
	\caption{Our evaluation methodology. We test whether a standard instruction prompt is enough to produce polarization in the form of \ac{ip} (Table~\ref{tab:inherent-polarization}), \ac{ndfu} (Fig.~\ref{fig:llm-polarization-adversarial}) and apunim (Tables~\ref{tab:llm-apunim-by-prompt-kumar},~\ref{tab:llm-apunim-by-prompt-sap}). We then repeat the analysis for four adversarial instruction prompts emphasizing the effect of personas.}
	\label{fig:llm-results}
\end{figure*}


\paragraph{Premise} Prior literature has shown that LLMs are not reliable annotators when using persona prompting; they tend to reproduce stereotypes \citep{lutz-etal-2025-prompt, liu-etal-2024-evaluating-large} and are unreliable for minority groups \citep{lutz-etal-2025-prompt}. Unlike prior work that investigated model performance \citep{gupta2024bias} or human--LLM agreement \citep{dong-etal-2024-llm, gomez-etal-2024-algorithmic, masud-etal-2024-hate}, we test whether attributable polarization exists regardless of faithfulness to the actual groups. 

\paragraph{Experimental Procedure} We investigate four\footnote{We used six models, but pruned two since they proved unreliable---see ablations in App.~\ref{app:llms}.} open-source LLMs with six annotators per item\footnote{It is possible that LLMs can produce polarized patterns when scaled up to dozens or hundreds of annotators. This however is both hard to check due to exorbitant computational costs (App.~\ref{app:llms}) and hard to scale up in realistic deployment scenarios.} supplied with personas sampled from our human dataset. Our findings support social science work which warns against generating human annotation data without substantial replication studies \citep{rossi_2024, kapania-etal-2025-examining}. Fig.~\ref{fig:llm-results} shows our overall evaluation methodology. See App.~\ref{app:llms} for the ablation experimental procedure, full results, model details, dataset selection, exploratory analysis, and comparison to human annotations.
...
\label{app:full}
%TODO: Export tables directly from code
