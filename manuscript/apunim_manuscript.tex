...
We investigate six open-source LLMs with six annotators per item supplied with personas sampled from our human dataset across a sample of 300 comments for the Sap and 1000 items for the Kumar dataset. Our results show \emph{no statistically significant attribution effects} and very small \emph{apunim} values for any of the groups in all models and datasets (see App.~\ref{app:llms}; Table~\ref{tab:llm-apunim-by-prompt-kumar},~\ref{tab:llm-apunim-by-prompt-sap}). Using four adversarial prompts progressively emphasizing the importance of the LLMs' personas (App.~\ref{app:prompts}), as instruction prompts are known to influence the effect of personas \citep{weeber-etal-2026-one}, also yields no results. Indeed, the adversarial prompts did not seem to push the \emph{apunim} values to any particular direction, suggesting that instructing the model is an ineffective way of eliciting \emph{any} polarizing behavior.\footnote{These prompts do however tend to increase \ac{ip}, although not to human levels---see App.~\ref{app:llms}; Table~\ref{tab:inherent-polarization}.}\footnote{We note that there may exist other ways to induce polarizing behavior in LLMs such as Supervised Fine-tuning, although this is outside the scope of this work.}
