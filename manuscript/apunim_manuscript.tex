...
\subsection{Does apunim recover a known effect?}
\label{sec:apunim:recovery}

Polarization and agreement metrics rest on different assumptions, which is why
they are rarely compared directly. Fixing the size of a group effect by
construction makes them commensurable, and we do so on synthetic annotations
(App.~\ref{app:comparison}). Two outcomes bear on how \S\ref{sec:results}
should be read. The \emph{sign} of apunim carries information agreement-based
attribution does not: a group that drives polarization and one that is merely
noisy internally both depart from agreement, and Krippendorff's $\alpha$ scores
them alike ($+.03$ vs.\ $+.02$), whereas apunim separates them ($+.32$ vs.\
$-.27$). Our significance test, however, is anti-conservative---it rejects at
$\alpha=.05$ for 40--62\% of samples containing no group effect---because the
null is the \emph{mean} of the reused random partitions
(App.~\ref{app:stats:pvalue}), which narrows with the iteration count rather
than with the evidence. A permutation p-value restores calibration.

\begin{finding}
	\label{finding:comparison}
	The sign of apunim separates between-group polarization from within-group
	disagreement, which agreement-based attribution conflates. Its p-values,
	however, are anti-conservative and should be recomputed by permutation.
\end{finding}
...
\paragraph{Comparisons with other methods} Finally, we compare against agreement-based attribution and against the original aposteriori unimodality test only on synthetic data, where the group effect is known by construction (App.~\ref{app:comparison}); on the datasets of \S\ref{sec:datasets} no ground truth exists against which to score them. That comparison also surfaced a calibration problem in our own significance test (Finding~\ref{finding:comparison}), which is the likeliest explanation for a pattern we could not previously account for---why some groups show statistically significant effects while others do not. The p-values in \S\ref{sec:results} should be read as descriptive until recomputed by permutation.
...
\section{Comparison with agreement-based attribution}
\label{app:comparison}

\paragraph{Setup} We generate annotations on a 1--5 scale for 200 items, with
the group effect fixed by construction: annotators in group B are shifted by
$\delta$ relative to group A, so $\delta=0$ means the grouping is irrelevant
and larger $\delta$ means it explains more of the polarization. Every method
receives identical data and is asked whether the grouping explains the
polarization; we report the rate at which each rejects at $\alpha=.05$ over 40
replications. Annotations use a full crossed design (every annotator rates
every item), which is the most favourable case for agreement metrics, as no
cells are missing.

\paragraph{Baselines} Krippendorff's $\alpha$ measures agreement rather than
attribution, so the agreement baseline is the within-group gain it implies:
split the annotators on the grouping and test whether $\alpha$ rises,
calibrated by a label permutation test so that it carries a p-value on the same
footing as apunim. We write this as $\Delta\alpha$. We also include the original
aposteriori unimodality test~\cite{pavlopoulos-likas-2024}, which flags a
comment that is polarized overall while every group is internally unimodal.

\begin{figure}[t]
	\centering
	\includegraphics[width=\textwidth]{metric_comparison.png}
	\caption{Detection rate for a group effect of known size $\delta$; $\delta=0$
	is the false-positive rate and the dotted line marks the nominal $\alpha=.05$.
	Version \texttt{1.0.5} of our library binned each sample over its own range,
	which inflates the \ac{ndfu} of any group that does not reach both ends of the
	scale; \texttt{1.0.6} bins every group and partition on one fixed grid.}
	\label{fig:metric-comparison}
\end{figure}

\paragraph{Power} All methods recover a large effect (Fig.~\ref{fig:metric-comparison}).
They separate in the two regimes that matter for real corpora. With 20
annotators per item, the original test never fires at all ($\le.05$ across every
$\delta$): it requires a group's \ac{ndfu} to be exactly zero, and with few
annotations per group that is a knife-edge condition, which is the concrete form
of the noise problem raised in \S\ref{sec:related:attribution}. With a 20\%
minority it recovers at most $.40$. Both apunim and $\Delta\alpha$ remain at
ceiling in both regimes.

\paragraph{Specificity} Power alone does not separate the methods, because a
metric can fire for the wrong reason. Table~\ref{tab:comparison-specificity}
scores three cases where the grouping is not the explanation. When group B
merely has higher variance around a shared mean---disagreement without
bimodality---$\Delta\alpha$ fires every time, and cannot distinguish that case
from genuine polarization ($+.024$ vs.\ $+.031$). Apunim also fires, but its
value is strongly \emph{negative} ($-.268$ vs.\ $+.322$), which is the reading
given in \S\ref{sec:apunim:dataset}: the group is polarized among itself. This
signed separation is what a polarization metric buys over an agreement metric,
and it is the main argument for preferring one.
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{Xrrrr}
		\toprule
		& \textbf{apunim} & \textbf{$\Delta\alpha$} & \textbf{orig.\ AU} & \textbf{should fire?} \\
		\midrule
		...
		\bottomrule
	\end{tabularx}
	\caption{Mean statistic in four scenarios (80 annotators/item, balanced
	groups, $\delta=2.5$ where applicable). Only apunim separates a group that
	drives polarization from one that is merely noisy, and it does so by sign.}
	\label{tab:comparison-specificity}
\end{table}

\paragraph{Calibration} The last two rows of Table~\ref{tab:comparison-specificity}
show apunim's statistic behaving correctly---it sits at zero when the grouping
explains nothing---while its p-value does not: it rejects in 40\% and 53\% of
those samples. The same is visible at $\delta=0$ in
Fig.~\ref{fig:metric-comparison}, where the false-positive rate runs from
$.40$ to $.62$ against a nominal $.05$, while $\Delta\alpha$ ($\le.03$) and the
original test ($\le.05$) are calibrated. The cause is that the null is the mean
of the reused random partitions (App.~\ref{app:stats:pvalue}); its standard
error falls as $1/\sqrt{t}$ in the iteration count $t$, so significance can be
obtained by computing longer. Holding the data fixed and raising $t$ from 50 to
1600 leaves the rejection rate at $\approx.45$ while the median p-value falls
from $.112$ to $.054$. Scoring the same statistic against the permutation
\emph{distribution} rather than its mean gives a false-positive rate of $.10$
while retaining power ($.93$ at $\delta=1$).

\paragraph{Effect of the binning fix} Version \texttt{1.0.5} of our library
passed a bin \emph{count} to \texttt{numpy}, which spreads bins over the range
of each sample. A group whose annotations do not reach both ends of the scale
then acquires empty bins between populated ones, which DFU reads as
multimodality. This is not symmetric: the size-matched random partitions mix
all groups and so span the scale, while a concentrated group does not, which
inflates $Pol_{obs}$ relative to $Pol_{apr}$ and drives Eq.~\ref{eq:apunim}
negative. On the data of Fig.~\ref{fig:metric-comparison} the statistic was
negative in 88--100\% of replications for $\delta \in [1.5, 2.5]$, where it must
be positive. Binning every sample on one grid fixed over the annotation scale
removes the inversion ($+.143$, $+.222$, $+.322$, $+.428$ for
$\delta = 1.5$ to $3.0$; negative in 0\%).
