...
Annotations are essential for subjective tasks, such as content moderation, toxicity, and \acf{hs} detection, which are especially important for protecting minority groups~\citep{un_hate_speech_targets, ucdavis_hate_speech}. Human annotation is however unscalable in practical scenarios (e.g., academic research on established datasets, social media deployment), which motivates the use of traditional ML and LLM-as-a-judge models for annotation. Simultaneously, these tasks were traditionally part of safety-tuning new LLMs using RLHF, but because of the cost incurred by manual, human annotation, current practice also incorporates RLAIF, where LLM systems are used to train other LLMs~\citep{sharma_rlaif}. The reliance on these models for annotation requires evaluation, which is currently insufficient. Specifically, current practice relies on \ac{iaa} or majority-vote~\citep{ivey-etal-2025-nutmeg, petra_2022_handling, van_der_Velden}, which may lead to biased and misconfigured systems (e.g., toxicity models disproportionally marking \ac{aa} speech as ``toxic''~\citep{sap-etal-2019-risk}), or discard \emph{fundamental} disagreements between minority groups and the majority~\citep{quaranto2022dog}, (see the example in Fig.~\ref{fig:disagreement-vs-polarization}).\footnote{The simulated experiments in this study were implemented by generating random annotations following the normal distribution with differing means and \ac{sd}---see App.~\ref{app:experiments}.}
...
One alternative is using \emph{polarization}, a clustering approach which correlates better with human judgments and is much less sensitive to different group sizes \citep{Pavlopoulos2023, pavlopoulos-likas-2024}. While we can detect polarization, there is little work on how to detect whether it is \emph{actually caused by marginalized groups}. Our work thus tries to answer the following \textbf{Main Research Question (MRQ): To what extent are LLM disagreements consistent with those of human annotators?} We answer this question through the following sub-questions:

\begin{researchquestion}[Why do current polarization representations fail]
	\label{rq:inherent}
	While prior work assumed that all polarization can be explained and attributed to some annotator characteristic, through the exhaustive creation of theoretical annotator groups (\S\ref{sec:inherent}) our study discovers the existence of \ac{ip}, which can not be attributed to \emph{any possible group} of annotators---known or unknown (Finding~\ref{finding:inherent}). We also discover that polarization across items has a \emph{direction}, preventing us from aggregating annotations on the dataset level to circumvent annotation sparsity (Finding~\ref{finding:conflicting}). We later show that this phenomenon limits existing statistical approaches. 
\end{researchquestion}

\begin{researchquestion}[How can we build a reliable representation of polarization attribution]
	\label{rq:metric}
	In order to solve these issues we introduce a new metric ``\emph{apunim}'', which \emph{quantitatively} attributes polarization to specific \acp{pc} and outperforms prior approaches in scenarios with realistic annotation sparsity and minority group splits (Finding~\ref{finding:comparison}). We apply our metric to four \ac{nlp} datasets focused on Toxicity and \ac{hs} detection, confirming prior work showing that annotator gender and ethnicity significantly impact annotation (Finding~\ref{finding:human}).
\end{researchquestion}

\begin{researchquestion}[Which polarization patterns are shared between human and LLM annotators]
	\label{rq:llm}
	We conduct an annotation study on one Toxicity and one \ac{hs} dataset using six open-source models in order to compare polarization patterns between them and humans (\S\ref{sec:results:llms}). We find that using models using persona-prompting generally fail to produce subgroup polarization patterns regardless of instruction prompt, confirming that persona prompting does not consistently influence LLM outputs in the examined tasks (Finding~\ref{finding:llms}).
\end{researchquestion}

Our work makes the following contributions:
\begin{enumerate*}
	\item The identification of severe issues present in subjective datasets and prior methodologies.
	\item A new polarization attribution metric that is robust to small annotator counts and minority group splits.
	\item An evaluation on how substantial LLM disagreements using persona prompting are in subjective annotation scenarios.
\end{enumerate*}
We also release a PyPI library implementing our metric.
...
We use four datasets from current \ac{nlp} research that include \ac{pc} information at the level of individual annotators; specifically, the ``Kumar'' \citep{kumar-et-al-2021} and ``Sap'' \citep{sap-etal-2022-annotators} datasets, which are concerned with Toxicity and \ac{hs} detection respectively. We also examine the DICES-350 and 990 datasets~\citep{dices} which focus on LLM response safety.\footnote{Specifically, we use the ``Breadth of Posts'' dataset with the ``\textit{toyou}'' labels for Sap. For DICES-350 we choose the \textit{Q3\_bias\_targeting\_inherited\_attributes} label, while for DICES-990 the \textit{Q3\_bias\_incites\_hatred} forgoing the \textit{Q3\_bias\_overall} labels which are aggregated scores. The choice of labels transforms DICES into a \ac{hs} task with the most coherence since the two datasets do not share labels.}

A brief overview of dataset characteristics is provided in Table~\ref{tab:datasets}. The Kumar dataset stands out for its large size, featuring more than 100{,}000 annotated items, as well as 10 distinct \ac{pc} dimensions.\footnote{We sample 1{,}000 items in order to allow for a full LLM annotation later. Our results remain consistent, for both different and differently sized-samples (App.~\ref{app:ablation}).} In contrast, the DICES datasets contain fewer annotated items but involve more than an order of magnitude more annotators per item than the other datasets. More details on the annotations, including overall levels of observed polarization, can be found in App.~\ref{app:ann_details}. For a discussion on the need for datasets with high annotator counts per item, instead of high annotator counts in general, see App.~\ref{app:ideal-dataset}.
...
Intuitively, a group partially explains the polarization of a item $c$ when the annotations of that group $A(c \given g) = \{a(c, g') \in A(c) \given g'=g\}$ exhibit higher polarization compared to the full set of annotations $A(c)$. Consider the case where a misogynistic comment is annotated for toxicity by men and women shown in Fig.~\ref{fig:inherent}, left.\footnote{This example can be generalized to $k$ groups; e.g., non-binary or transgender annotators.} The annotations are generally polarized ($nDFU_{all}=0.62$); but both women ($nDFU_{women}=0.10$) and men ($nDFU_{men}=0.37$) exhibit low polarization between their respective groups, suggesting that the overall polarization is partially caused by substantive disagreements between the two examined genders.
...
\acf{ip} refers to disagreement that cannot be attributed to \emph{any arbitrary} group. Prior work~\citep{pavlopoulos-likas-2024} attributed polarization to groups only when polarization for each individual group became exactly zero (e.g. $nDFU(all) > nDFU(men)=nDFU(women) = 0$). The presence of \ac{ip} would invalidate this theory, since there would be no possible group or combination of groups $g \textit{ s.t. } nDFU(g) = 0$. Formally, the \ac{ip} for a single item $c$ is the minimum polarization exhibited by any arbitrary group (Eq.~\ref{eq:inherent}).
\begin{equation}
	\label{eq:inherent}
	Pol_{inh}(c) =
...
When considered exhaustively, these theoretical, random splits produce both observed and latent groups (e.g., annotators may be less strict if they had lunch prior to annotation). Since \ac{ndfu} operates only when a group has at least three annotators (Eq.~\ref{eq:ndfu}), this yields $\sum_{k=3}^{n} \binom{n}{k}$ possible groups. Since enumerating all subsets is computationally intractable for large annotator counts, we use Monte Carlo sampling by drawing $k$ random partitions with random sizes (Eq.~\ref{eq:random}) which serves as an upper bound (as it considers a strict subset of Eq.~\ref{eq:inherent}).
...
Finding the reason behind \ac{ip} is difficult given the lack of datasets (see App.~\ref{app:inpol} for some hypotheses and further analysis). If we assume that \ac{ip} is caused by a lack of annotations per item, we could address the issue by aggregating annotations between items. However, Fig.~\ref{fig:ndfu_discussion} shows that two items polarized for the opposite reason may lead to a false negative. Therefore, any statistics must never be aggregated across items.\footnote{A similar assumption was followed in early parametric approaches, which assumed different parameters for each item~\citep{checco_2017}.}
...
\begin{multicols}{2}
	...
	\end{equation}
\end{multicols}
\noindent where $\tilde{r}$ is the random partition operator (see Eq.~\ref{eq:inherent}). We explain why defining apriori polarization in this way is preferable to alternatives in App.~\ref{app:design}.
...
As shown in Fig.~\ref{fig:metrics}, \emph{both the $\chi^2$ and GMM methods generally fail} at attributing polarization. \ac{iaa} remains competitive when many annotators are available, but underperforms in cases where the examined group is a minority and when a few annotators are available per item (a common case among \ac{nlp} datasets---see \S\ref{sec:datasets}). Apunim in contrast is robust both to the number of annotators per item, opposing polarization effects, and minority groups, validating our initial motivation.

We also observe that the simple statistical approaches do indeed show excellent performance when polarization is unidirectional. This is expected, since they rely on consistent fixed means effects, which is disrupted by opposing polarization effects (see Fig.~\ref{fig:ndfu_discussion}) violating their assumptions on fixed mean differences between groups. Thus, while these methods perform well in ideal conditions, they may completely fail on realistic annotation datasets. While we consider two groups in this experiment, ablations with $k=\{3,5,10\}$ groups can be found in App.~\ref{app:ablation}; Fig.~\ref{fig:metrics-multiple}.
...
In order to lower the computational cost required, we re-use the random partitions $\tilde{r}_i$, which are originally calculated to obtain the apriori polarization values (see Eq.~\ref{eq:apriori}), for the p-value calculations (App.~\ref{app:pvalue}). This means that in our current implementation, the number of groups for estimating apriori polarization ($\tilde{r}_i$ in Eq.~\ref{eq:apriori}), and the number of apunim values used in the p-value calculation have to be the same.

We use version \texttt{1.0.6} of the \texttt{apunim} library for our experiments. We use 100 random iterations for the apriori value calculation (Eq.~\ref{eq:apriori}) and the p-value estimation (App.~\ref{app:pvalue}), and set a confidence level of $\alpha=0.05$. The relative library setup is provided as a \texttt{conda} environment file to ensure reproducibility.
...
\paragraph{Defining Apriori Polarization} In \S\ref{sec:inherent:formulation}, we explained how our methodology needs to compare the polarization of $A(c)$ and $A(c \given g)$. Direct comparisons are not advisable, since we would be comparing statistics between a set and its subset---in this case, standard practice dictates comparing the subset with its complement; i.e., the in-group vs. the out-group instead of the in-group vs. all the annotations. However, this also would not work; if we apply the subset-vs-complement method in the example of Fig.~\ref{fig:inherent}, left, we would end up subtracting the \acp{ndfu} of men and women, which are similar as they are both unimodal distributions, falsely concluding that there is no polarization present.
...
\paragraph{Filtering Datasets with Desirable Levels of Subjectivity} In order to investigate whether the numerous annotators per items can explain the lack of \ac{ip} in the DICES datasets (\S\ref{sec:inherent}) we gradually subsample the annotators for each item randomly with replacement, starting from five annotators per item, and scaling up to the full complement of annotators in the dataset with a step of two. For each such step, we choose ten annotator samples and compute the maximum \ac{ip} across all runs. Even with annotator counts similar to the Kumar and DICES datasets, \ac{ip} remains zero, suggesting that the absence of \ac{ip} can not be attributed to annotator number alone---rather, it is plausibly a pattern attributable to the DICES datasets themselves. This kind of analysis may be used in the exploratory phase of dataset selection in order to filter out datasets without enough group-specific subjectivity.
