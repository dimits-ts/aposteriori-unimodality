...
No. We investigated polarization attribution patterns for LLM annotators  across seven open-source models.\footnote{We exclude OLMo-7B and LLaMa-8B due to prompt sensitivity and do not include the DICES datasets due to the need for hundreds of simulated annotators per item.} Specifically, we used six annotators per item supplied with personas sampled from our human dataset for a random sample of 300 comments for the Sap dataset and 1000 items for Kumar. We find no statistically significant attribution effects and very small \emph{apunim} values for any of the groups in all models and datasets (see App.~\ref{app:llms};  Table~\ref{tab:llm-apunim-by-prompt-kumar},~\ref{tab:llm-apunim-by-prompt-sap}). 

We also attempted to use two adversarial prompts emphasizing the importance of the LLMs' personas (App.~\ref{app:prompts}) without success. Indeed, the adversarial prompts did not seem to push the \emph{apunim} values to any particular direction, suggesting that instructing the model is an ineffective way of eliciting \emph{any} polarizing behavior. This pattern can be explained both by the lack of overall polarization exhibited by these models and the homogeneous distribution of polarization across groups, which are not observable to this extent in human annotations (Fig.~\ref{fig:llm-polarization}). See App.~\ref{app:llms} for the ablation experimental procedure, full results, model details, dataset selection, exploratory analysis, and comparison to human annotations.
...
Tables~\ref{tab:llm-apunim-by-prompt-kumar} and~\ref{tab:llm-apunim-by-prompt-sap} show the apunim results for the LLM annotation task on the Kumar and Sap datasets respectively.
