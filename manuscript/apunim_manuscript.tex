...
\begin{figure*}[t]
	...
	\caption{\ac{ndfu} grouped by dataset (rows) and model family (columns) for each \ac{pc} persona dimension in the primary annotation task using the default prompt.}
	\label{fig:llm-polarization}
\end{figure*}

Fig.~\ref{fig:llm-exploratory} shows the polarization exhibited by each model for each dataset representing each \ac{pc} dimension. Polarization is largely model-dependent instead of dataset-dependent. Specifically, large models (32B-70B parameters) tend to not show systematic differences in opinion due to persona prompting. Furthermore, the patterns in polarization tend to be uniformal; only a few groups show frequent changes in polarization patterns, which is not met in human annotations (Fig.~\ref{fig:dataset-polarization}).
