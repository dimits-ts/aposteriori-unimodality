...
%TODO: Flip narrative
\begin{abstract}
	LLM-as-judge systems are increasingly used as scalable substitutes for human annotators, including in subjective tasks where human annotators systematically disagree along group lines---a phenomenon known as polarization. Whether these systems reproduce, ignore, or distort human polarization patterns is largely untested, in part because existing polarization metrics are unreliable. We find that they rest on a faulty assumption---that all polarization can be attributed to some annotator group---and that they can miss polarization entirely when opposing group effects cancel out in aggregate. We introduce a non-parametric clustering method that resolves both failure modes, enabling quantifiable attribution of polarization to any annotator group without training. Using this method, we audit LLM-as-judge systems against real human annotation datasets and find that their polarization structure is largely disconnected from human patterns, a gap that persists even when models are explicitly prompted to condition on persona characteristics. We validate the underlying method on simulated data, showing it remains reliable under group-size imbalance and inconsistent polarization effects that degrade current approaches. Our results suggest that persona-conditioning alone is insufficient for LLM judges to replicate the structure of human disagreement, with implications for their use in RLHF and other pipelines that treat human feedback as a proxy signal.
\end{abstract}
...
\begin{researchquestion}[Which polarization patterns are detectable using our new representation]
	\label{rq:human}
	We apply our metric to four \ac{nlp} datasets focused on Toxicity and \ac{hs} detection (\S\ref{sec:results:humans}), confirming prior work showing that annotator gender and ethnicity significantly impact annotation (Finding~\ref{finding:human}).
\end{researchquestion}
