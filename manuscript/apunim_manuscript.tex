...
\title{Diagnosing and Fixing Polarization Attribution to Groups in Subjective Annotations}
...
\begin{figure}[t]
	\centering
	\includegraphics[width=\textwidth]{disagreement_vs_polarization.png}
...
\begin{researchquestion}[Which polarization patterns are shared between human and LLM annotators]
	\label{rq:llm}
	We conduct an annotation study on the same datasets using seven open-source models in order to compare polarization patterns between them and humans (\S\ref{sec:results:llms}). We find that persona-prompted LLMs consistently fail to produce any subgroup polarization patterns, confirming that persona prompting does not consistently influence LLM outputs (Finding~\ref{finding:llms}).
\end{researchquestion}

Our work thus makes the following contributions: (1) The identification of severe issues present in subjective datasets and prior methodologies (2) A new polarization attribution metric that is robust to small annotator counts and minority group splits with a PyPi-installable implementation (3) The detection of new promising directions (ideological and ordinal groups) for analysis of subjective \ac{nlp} datasets (4) An exploration of LLM polarization patterns when using persona prompting. Our study also underscores the need for datasets with high annotator counts per item, instead of high annotator counts in general---see App.~\ref{app:ideal-dataset} for a more extensive discussion and empirical findings.
...
\begin{figure}[t]
	\centering
	\includegraphics[width=0.45\textwidth, valign=t]{ndfu_combined.png}
	\includegraphics[width=0.45\textwidth, valign=t]{apriori.png}
...
We compute the \ac{ip} for each item across the four datasets. For Sap and Kumar, we use the exhaustive formulation (Eq.~\ref{eq:inherent}), whereas for the two DICES datasets we rely on the Monte Carlo approximation with $k=1000$ (Eq.~\ref{eq:random}). Fig.~\ref{fig:inherent}, right shows that in all DICES items there exists at least one group that fully accounts for the observed polarization.\footnote{$Pol_{inh}^{MC}(c)=0 \Rightarrow Pol_{inh}(c)=0$ due to Eq.~\ref{eq:lower-bound}, as is the case of increased $k$ in Eq.~\ref{eq:random}.} Interestingly, this observation does not hold for the Kumar and Sap datasets, despite evaluating all possible annotator groups exhaustively. See Table~\ref{tab:inherent-examples}; App.~\ref{app:ann_details} for a qualitative look through items with high InPol.
...
\begin{figure}[t]
	\centering
	\includegraphics[width=\textwidth]{apunim_ordinal.png}
	\caption{Polarization attribution trends in ordinal groups. The left side of the x-axis refers to young annotators, low education, low religiousness, and negativity towards the impacts of toxicity and technology. The full ordinal modes for each dimension and their individual labels can be found in App.~\ref{app:full}. We exclude groups with $<50$ annotations, since they near-universally result in zero apunim values due to lack of data. Filled points represent statistical significant ($p<0.05$) values.}
	\label{fig:ordinal}
\end{figure}
...
\subsection{What Polarization Patterns Do LLM Annotators Exhibit?}
\label{sec:results:llms}

\begin{figure*}[t]
	\centering
	\includegraphics[width=\textwidth]{llm_apunim_grid.png}
	\caption{\ac{ndfu} grouped by dataset (rows) and model family (columns) for each \ac{pc} persona dimension in the primary annotation task (blue) using the default prompt, compared with human annotations on the same sample (yellow).}
	\label{fig:llm-polarization}
\end{figure*}

Prior literature has shown that LLMs are not reliable annotators when using persona prompting; they tend to reproduce stereotypes \citep{lutz-etal-2025-prompt, liu-etal-2024-evaluating-large}, are unreliable for minority groups \citep{lutz-etal-2025-prompt} and persona adherence has been shown to be volatile \citep{shu-etal-2024-dont, kovavc2024stick}. Unlike prior work that investigated model performance \citep{gupta2024bias} or human--LLM agreement \citep{dong-etal-2024-llm, gomez-etal-2024-algorithmic, masud-etal-2024-hate}, we aim to discover whether attributable polarization exists regardless of faithfulness to the actual groups.

We investigate six open-source LLMs with six annotators per item supplied with personas sampled from our human dataset. We annotate a random sample of 300 comments for the Sap dataset and 1000 items for Kumar. Our results show \emph{no statistically significant attribution effects} and very small \emph{apunim} values for any of the groups in all models and datasets (see App.~\ref{app:llms}; Table~\ref{tab:llm-apunim-by-prompt-kumar},~\ref{tab:llm-apunim-by-prompt-sap}). Using four adversarial prompts progressively emphasizing the importance of the LLMs' personas (App.~\ref{app:prompts}), since instruction prompts are known to influence the effect of personas \citep{weeber-etal-2026-one}, also yields no results. Indeed, the adversarial prompts did not seem to push the \emph{apunim} values to any particular direction, suggesting that instructing the model is an ineffective way of eliciting \emph{any} polarizing behavior. \footnote{These prompts do however tend to increase \ac{ip}, although not to human levels---see App.~\ref{app:llms}; Table~\ref{tab:inherent-polarization}.}\footnote{We note that there may exist other ways to induce polarizing behavior in LLMs such as Supervised FineTuning, although this is outside the scope of this work.}

This pattern can be explained both by the lack of overall polarization exhibited by these models and the homogeneous distribution of polarization across groups, which are not observable to this extent in human annotations (Fig.~\ref{fig:llm-polarization}), which is further supported by overall small \ac{ip} values (App.~\ref{app:llms}; Table~\ref{tab:inherent-polarization}). Our findings therefore support social science work which warns against generating human annotation data without substantial replication studies \citep{rossi_2024, kapania-etal-2025-examining}.
...
\begin{finding}
	\label{finding:llms}
	LLMs with persona-prompting do not produce annotations with attributable polarization across any group, which may be explained by the even distribution of (fewer) polarization across simulated groups.
\end{finding}
...
We ensure that our LLM annotation study is reproducible by selecting exclusively open-source, locally hosted models and fixing all relevant seeds. An ablation study where we repeat annotations multiple times shows that our setup achieves 100\% consistency across all models. In order to ensure that the LLM annotators are not influenced by minute differences in the prompts, we execute a second ablation study where we rephrase the main annotation prompt three times and compare outputs. This procedure revealed that two models were especially sensitive to such changes, which led us to exclude those models from the analysis presented in this manuscript. Information on the synthetic experiments used in \S\ref{sec:inherent},\ref{sec:apunim} is disclosed in App.~\ref{app:experiments}.

We analyze software design decisions, limitations and computational details in App.~\ref{app:software} and elaborate on our experimental setup in App.~\ref{app:design}, while ablations around sample sizes and sample selection for the Kumar dataset are detailed in App.~\ref{app:ablation}. Additionally, we include a discussion around the use of current \ac{nlp} datasets, as well as an empirical estimation of the number of required annotators for reproducible results in App.~\ref{app:ideal-dataset}.  All code, data-processing scripts and the library implementing \emph{apunim} are released anonymously: library \librarylink, online (HTML) documentation \doclink, replication code \explink.
...
\begin{figure}[t]
	\includegraphics[width=\textwidth]{kumar3k_seed_ablation.png}
	\centering
...
\begin{figure}[t]
	\centering
	\includegraphics[width=\textwidth]{pol_obs_subsampling_robustness.png}
...
\begin{figure*}[t]
	\centering
	\includegraphics[width=\textwidth]{human_vs_llm_histograms.png}
